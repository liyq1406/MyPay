...
//  YunRichMPCR
//
//  Created by YunRich on 15/5/19.
//  Copyright (c) 2015年 YunRich. All rights reserved.
//

#import "YRDealDetailModel.h"

@implementation YRDealDetailModel 

+ (void)queryDealDetailWithDateType:(DateDurationType)dateType pageNum:(NSInteger)pageNum successBlock:(void (^)(id))successBlock failureBlock:(void (^)(NSString *))failureBlock {
    ...
    NSString * pages = [NSString stringWithFormat:@"%ld",(long)pageNum];
    YRTransactionRequest * transactionRequest = [[YRTransactionRequest alloc] init];
    NSDictionary * parameters = @{@"BEGINDATE" :startDate ,
                                  @"ENDDATE" : currentDate,
                                  @"PAGESIZE" : @"20",
                                  @"PAGENUM" : pages
                                  };
    [transactionRequest queryTransactionRecords:parameters successBlock:^(id responseObject) {
